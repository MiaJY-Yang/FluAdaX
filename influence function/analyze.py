...
from pipeline import construct_mega, get_aiv_dataset
...
def main():
    ...
    # Prepare the dataset.
    train_dataset = get_aiv_dataset(
        data_name=args.dataset_name,
        split="train",
    )
    eval_dataset = get_aiv_dataset(
        data_name=args.dataset_name,
        split="test",
    )
    ...
    model = construct_mega()
    checkpoint_path = os.path.join(args.checkpoint_dir, "model_simple.pth")
    ...
    score_args = ScoreArguments(query_gradient_rank=rank, query_gradient_svd_dtype=torch.float32, per_module_score=True)
